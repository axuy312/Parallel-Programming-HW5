...
__global__ void mandelKernel(int* device_img, int width, float stepX, float stepY, float lowerX, float lowerY, int count, int pitch) {
...
void hostFE (float upperX, float upperY, float lowerX, float lowerY, int* img, int resX, int resY, int maxIterations)
{
    ...
    mandelKernel<<<numBlocks, numThreads>>>(device_img, resX, stepX, stepY, lowerX, lowerY, maxIterations, pitch);
